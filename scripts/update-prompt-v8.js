/**
 * Retell LLM v8: Airtable-Migration, reschedule_appointment hinzugefügt
 */
...
5. Bei success=false: Die message vorlesen und warten was der Gast moechte.
6. Bei success=true: Bestaetigung vorlesen und bedanken.

UMBUCHUNG (Termin verschieben):
Wenn Gast Termin verschieben moechte:
1. Neues Datum und neue Uhrzeit erfragen.
2. reschedule_appointment EINMAL aufrufen — kein cancel + book mehr, nur reschedule.
3. Bei success=true: neue Buchungsnummer vorlesen (aus new_id).
4. Bei success=false: message wortwoertlich vorlesen.
WICHTIG: NIEMALS manuell cancel_appointment + book_appointment fuer Umbuchungen kombinieren. Immer nur reschedule_appointment nutzen.
...
const tools = [
  ...
  {
    execution_message_description: 'Ich verschiebe Ihre Reservierung.',
    speak_after_execution: false, name: 'reschedule_appointment',
    description: 'Verschiebt eine bestehende Reservierung auf neues Datum und Uhrzeit. Sucht automatisch per Telefonnummer des Anrufers. Gibt success, new_id, message zurueck.',
    type: 'custom', speak_during_execution: true,
    parameters: {
      type: 'object',
      properties: {
        confirmation_id: { type: 'string', description: 'Buchungs-ID z.B. MSTR-4821 (optional wenn Telefon bekannt)' },
        guest_name:      { type: 'string', description: 'Name des Gastes (optional)' },
        new_date:        { type: 'string', description: 'Neues Datum YYYY-MM-DD' },
...
async function main() {
  ...
  console.log('✓ LLM v8 aktualisiert — Tools:', toolNames.join(', '));
}
