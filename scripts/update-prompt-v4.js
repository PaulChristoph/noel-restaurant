/**
 * Retell LLM v4: lookup_reservation Tool hinzufügen + System-Prompt Update
 */
...
Wenn ein Gast fragt: "Wann habe ich reserviert?", "Ich habe eine Buchung" oder eine Buchungsnummer nennt:
1. lookup_reservation aufrufen.
   - Falls Buchungsnummer genannt (z.B. MSTR-4821): confirmation_id Parameter verwenden.
   - Falls nur Name: guest_name Parameter verwenden.
   - Falls Gast sagt "ich rufe von meiner Nummer an": kein Parameter noetig, System erkennt Nummer automatisch.
2. Ergebnis vorlesen: Datum, Uhrzeit, Personenzahl, Buchungsnummer.
3. Fragen: Kann ich noch etwas fuer Sie tun? z.B. Aenderung oder Stornierung?
...
10. Fragen: Moechten Sie auf der Nummer, mit der Sie gerade anrufen, eine Bestaetigungs-SMS erhalten?

    WENN JA:
    - Falls detected_phone vorhanden: send_confirmation aufrufen (method=sms, contact=detected_phone).
    - Falls kein detected_phone: Auf welche Nummer soll ich die SMS schicken?
      Nummer wiederholen: Also null, eins, sieben... Korrekt? Dann send_confirmation aufrufen.

    WENN NEIN: Okay, kein Problem.

11. Fragen: Darf ich sonst noch etwas fuer Sie tun?
12. Bei Nein oder Verabschiedung: Vielen Dank, wir freuen uns auf Ihren Besuch. Auf Wiederschauen! Dann end_call.
...
const tools = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        contact: { type: 'string', description: 'Telefonnummer oder E-Mail des Gastes' },
  ...
  },
  {
    ...
    description: 'Sucht eine bestehende Reservierung per Name, Buchungs-ID oder automatisch per Anrufernummer. Aufrufen wenn Gast nach seiner Buchung fragt.',
    type: 'custom',
    speak_during_execution: true,
    parameters: {
      type: 'object',
      properties: {
        guest_name: { type: 'string', description: 'Name des Gastes (optional)' },
        confirmation_id: { type: 'string', description: 'Buchungs-ID z.B. MSTR-4821 (optional)' },
      },
    },
    url: `${BASE_URL}/retell/function-call`,
  },
...
async function main() {
  ...
  console.log('✓ LLM v4 aktualisiert');
  console.log('Tools:', toolNames.join(', '));
  console.log('lookup_reservation:', toolNames.includes('lookup_reservation') ? 'JA ✓' : 'NEIN ✗');
}
