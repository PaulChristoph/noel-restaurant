/**
 * Retell LLM v6: Storno-Schleife fix — cancel_appointment wird nur einmal aufgerufen
 */
...
4. Ergebnis direkt vorlesen — egal ob Erfolg oder Misserfolg. Nicht wiederholen.
5. Bei Misserfolg: Meldung vorlesen und fragen ob Gast die Buchungsnummer (MSTR-XXXX) nennen kann.
...
5. Bei Verfuegbarkeit: cancel_appointment fuer alten Termin EINMAL aufrufen (WICHTIG: erst stornieren!).
6. book_appointment fuer neuen Termin aufrufen.
7. Bestaetigen: Umgebucht von [alt] auf [neu], neue Buchungs-Nr. [ID].
...
const tools = [
  ...
  },
  {
    ...
    description: 'Storniert eine bestehende Reservierung. Auch bei Umbuchung zuerst aufrufen um den alten Termin zu loeschen.',
    type: 'custom', speak_during_execution: true,
...
async function main() {
  ...
  console.log('✓ LLM v6 aktualisiert — Tools:', toolNames.join(', '));
}
